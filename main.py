    ...
    def update(self):
        ...
        if not self.ended:
            if self.main_delay > 0 or self.current_delay > 0:
                ...
                #segment delay, iterations, entity spawned, number, acceleration, direction, speed, startpoint, endpoint
                if self.current_segment_iteration < self.spawn_dictionary[self.current_phase][self.current_segment][1]:
                    ...
                    self.entity_amount = self.spawn_dictionary[self.current_phase][self.current_segment][3]
                    self.spawning_x = self.spawn_dictionary[self.current_phase][self.current_segment][7][0]
                    self.spawning_y = self.spawn_dictionary[self.current_phase][self.current_segment][7][1]
                    self.spawn_acc_x = str(self.spawn_dictionary[self.current_phase][self.current_segment][4][0])
                    self.spawn_acc_y = str(self.spawn_dictionary[self.current_phase][self.current_segment][4][1])
                    self.spawn_direction = str(self.spawn_dictionary[self.current_phase][self.current_segment][5])
                    self.spawn_speed = str(self.spawn_dictionary[self.current_phase][self.current_segment][6])
                    self.increment_x = ((self.spawn_dictionary[self.current_phase][self.current_segment][8][0] - self.spawning_x)/self.entity_amount)
                    self.increment_y = ((self.spawn_dictionary[self.current_phase][self.current_segment][8][1] - self.spawning_y)/self.entity_amount)
                    for i in range(self.entity_amount):
                        living_sprites.add(self.entity(self.spawning_x, self.spawning_y, eval(self.spawn_acc_x), eval(self.spawn_acc_y), eval(self.spawn_direction), eval(self.spawn_speed)))
...
class Player(pygame.sprite.Sprite):
    ...
    def move(self, x_factor, y_factor):
        ...
        if x_factor != 0 and y_factor != 0:
            self.x_position += x_factor*(1/sqrt(2))*speed
            self.y_position += y_factor*(1/sqrt(2))*speed
        else:
...
class BasicBullet(Bullet):
    ...
    def clean(self):
        if self.x_position > DISPLAY_WIDTH + 12:
            self.kill()
        elif self.x_position < -12:
            self.kill()
        elif self.y_position > DISPLAY_HEIGHT + 12:
            self.kill()
        elif self.y_position < -12:
            self.kill()
    def phase_end(self):
        bullet_particles(self.x_position, self.y_position, 1)
        self.kill()
...
class BigBullet(Bullet):
    ...
    def update(self):
        ...
        self.location = self.x_position, self.y_position
    def clean(self):
        if self.x_position > DISPLAY_WIDTH + 40:
            self.kill()
        elif self.x_position < -40:
            self.kill()
        elif self.y_position > DISPLAY_HEIGHT + 40:
            self.kill()
        elif self.y_position < -40:
            self.kill()
    def phase_end(self):
        bullet_particles(self.x_position, self.y_position, 1)
        self.kill()
...
class CollideBox(pygame.sprite.Sprite):
    ...
    def clean(self):
        if self.x_position > DISPLAY_WIDTH + 50:
            self.kill()
        elif self.x_position < -50:
            self.kill()
        elif self.y_position > DISPLAY_HEIGHT + 50:
            self.kill()
        elif self.y_position < -50:
            self.kill()
...
def get_spawn_dictionary():
    ...
    #segment delay, iterations, entity spawned, number, acceleration, direction, speed, startpoint, endpoint
    spawn_dictionary = [
        [[2, "collect", 4, [(400,300), (1520, 150), (400,780), (1520,780)]], [0.75, 2, BasicBullet, 25, (0,0), 3*pi/2, 400, (1920,0), (1920,1080)], [0.75, 2, BasicBullet, 25, (0,0), pi/2, 400, (0,22), (0,1102)]],
        [[2, "collect", 4, [(400,300), (1520, 150), (400,930), (1520,930)]], [0.75, 6, BasicBullet, 25, ("300*(rand.random()-0.5)",-150), "pi*(rand.random()-0.5)", 500, (0,1080), (1920,1080)], [0.75, 6, BasicBullet, 25, ("300*(rand.random()-0.5)",150), "pi+(pi*(rand.random()-0.5))", 500, (0,0), (1920,0)]],
        []
    ]
    maximum_phase = 1
    return spawn_dictionary, maximum_phase
